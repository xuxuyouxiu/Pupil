...
### 修复
- **启动后球迟迟不出现（托盘先出、球晚几秒）**：实测本机 Electron 引擎 106ms 即 ready，瓶颈不在引擎而在启动串行链——core.start() 的同步段（SQLite 打开、8 个适配器启动、外部 adapter 扫描）与渲染首帧争主线程。优化：core.start() 改为真正延后执行（球窗创建即返回），完成后补发快照广播给球窗（新增公开 broadcastNow 与 onCoreReady 回调）；另加 1.5s 强制显示兜底——ready-to-show 偶发不发（杀软拦截渲染）时球不会再永远缺席，最多空白一瞬

## [1.8.0] - 2026-08-28

> **版本号修正版**：内容与 1.1.3 完全一致（插话打断误报修复）。此前版本号被错误地
> 从 1.7.0 序列切回 1.1.x，导致已装 1.7.0 的用户检查更新时更新器判定 1.1.3 为旧版本
> 而拒绝——electron-updater 依赖版本号单调递增。此错误为发版流程人为失误，此后严守
> 版本号只增不减。**从 ≤1.6.x 升级的用户不受影响**。

### 修复（同 1.1.3）
- **插话时误响「出错」音效**：ZCode 在用户插话时主动打断当前轮次（preempts active turn 等），被打断请求在日志中记为 error——Pupil 误将用户主动行为当故障报错。新增错误三级分类：打断类不发 error；瞬态类（并发/网络，自动重试）不响出错音只轻提示；其余正常提醒
